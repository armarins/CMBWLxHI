...
#
#BINNING=True
PATH_TO_SHEAR_PARAMS_FILE="/data/AMARINS/LSSxHI-CODES/scripts/parameters/lsst_desc_parameters.yaml"
SURVEY_SHEAR="LSST"
DATASET_SHEAR="Y10"
BINNING=1
#
...
############
for NBINS in 5 10 15 20 40 50 70; do 
    max_j=$((NBINS - 1))  # Calculate i + 5
    for j in $(seq 0 $max_j); do
        PREFIX_new="${PREFIX}${j}_${SIGMA_Z}_tomos${NBINS}"
        python $PATHFILE --verbose $VERBOSE --pathout $PATHOUT --prefix $PREFIX_new --suffix $SUFFIX\
                         ...
                         --nbins_shear ${NBINS} --sigmaz_shear ${SIGMA_Z} --l_max ${LMAX}| tee -a $FILEOUT
    done
done
